<h1 align="center">Hi 👋, I'm Chitraruban S R</h1>

<h3 align="center">
Java Developer | Final Year Electronics & Communication Engineering Student
</h3>

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&size=28&pause=1000&center=true&vCenter=true&width=700&lines=Java+Developer;Problem+Solver;Open+Source+Learner;Always+Learning+New+Things+🚀">
</p>

---

# 👨‍💻 About Me

🎓 Final Year Electronics & Communication Engineering Student

💻 Passionate Java Developer

🌱 Currently learning Data Structures & Algorithms

🚀 Interested in Software Development

🎯 Looking for Software Engineer opportunities

---

# 💻 Tech Stack

<p align="center">

<img src="https://skillicons.dev/icons?i=java,html,css,js,mysql,git,github,vscode"/>

</p>

---

# 🛠 Skills

- ☕ Java
- 🌐 HTML
- 🗄️ MySQL
- 🔧 Git & GitHub
- 💻 VS Code
- 👥 Leadership
- 🤝 Team Management

---


# 🎓 Education

### B.E. Electronics & Communication Engineering

🏫 VSB Engineering College

📈 CGPA: **7.9**

---

# 💼 Internship

## Brainery Spot Technologies

**Full Stack Web Development Intern**

- Completed a 30-day Full Stack Web Development Internship.
- Developed a Student Management System.
- Implemented CRUD Operations.
- Worked with Frontend, Backend and Database.

---

# 🚀 Featured Projects

<details open>
  
<summary><b>🚗 V8 Engine with Alcohol Detector</b></summary>

### 📖 Overview
Developed a vehicle safety system that prevents the engine from starting if alcohol is detected above the predefined limit. The system also provides visual and audible alerts.

### ✨ Key Features
- 🍺 Alcohol Detection
- 🚫 Automatic Engine Lock
- 🔔 Buzzer Alert
- 💡 LED Status Indicator

### 🛠️ Tech Stack

![Embedded C](https://img.shields.io/badge/Embedded_C-blue?style=flat-square)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat-square&logo=arduino)
![NodeMCU](https://img.shields.io/badge/NodeMCU-2C3E50?style=flat-square)
![MQ-3 Sensor](https://img.shields.io/badge/MQ--3_Alcohol_Sensor-success?style=flat-square)

</details>

---

<details>
  
<summary><b>🧘 AI Yoga & Health Tracking Assistant</b></summary>

### 📖 Overview

AI-powered health assistant that recommends yoga practices and monitors health progress.

### ✨ Key Features
- 🤖 AI Recommendation
- ❤️ Health Tracking
- 🧘 Yoga Suggestions
- 📈 Progress Monitoring

### 🛠️ Tech Stack

![AI](https://img.shields.io/badge/AI-6A5ACD?style=flat-square)
![HTML](https://img.shields.io/badge/HTML-E34F26?style=flat-square&logo=html5)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=flat-square&logo=css3)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript)

</details>

---

<details>
  
<summary><b>📍 Smart Tourist Management System</b></summary>

### 📖 Overview
IoT-based tourist safety system with live GPS tracking.

### ✨ Key Features
- 📍 Live Location
- 🌍 GPS Tracking
- 📡 IoT Communication
- 🔒 Improved Safety

### 🛠️ Tech Stack

![ESP32](https://img.shields.io/badge/ESP32-red?style=flat-square)
![GPS](https://img.shields.io/badge/GPS-blue?style=flat-square)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat-square&logo=arduino)

</details>

---

# 🏅 Achievements

<details open>
<summary><b>🏆 Sports Achievements</b></summary>

🥋 **State Level Kho Kho Player**

🚴 **District Second Place – Cycling**

🤸 **District Third Place – Pole Vault**

🧘 **Recognized for Excellence in Yoga**

</details>

---

<details>
<summary><b>💻 Technical Achievements</b></summary>

☕ **Java Programming Certification – GUVI**

🗄️ **Database & SQL Certification – Infosys Springboard**

💼 **Completed Full Stack Web Development Internship at Brainery Spot Technologies**

🚀 **Developed AI & IoT-Based Academic Projects**

</details>

---

<details>
<summary><b>🎯 Career Goals</b></summary>

✅ Master Java Development

✅ Strengthen Data Structures & Algorithms

✅ Build Real-World Software Projects

✅ Secure a Software Engineer Role

</details>
# 📫 Contact

📧 **Email:** rupan.pappani@gmail.com

💼 **LinkedIn:** https://www.linkedin.com/in/chitraruban-S-R

💻 **GitHub:** https://github.com/Chitraruban

🧩 **LeetCode:** https://leetcode.com/Chitraruban

---

<div align="center">

### 🚀 "Code • Learn • Build • Grow"

**Thank you for visiting my GitHub profile! ⭐**

</div>
